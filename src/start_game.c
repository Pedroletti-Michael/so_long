...
int	load_sprites(t_vars *vars, t_sprite *sprite)
{
	...
	if (sprite->player == NULL || sprite->enemies == NULL)
		return (error("Memory error allocation in module.map!"));
...
int	start_game(t_data *module)
{
	...
	if (module->vars->mlx == NULL)
		return (error("It is not possible to start the game!"));
	...
	if (module->vars->win == NULL)
		return (error("It is not possible to open the window of the game!"));
